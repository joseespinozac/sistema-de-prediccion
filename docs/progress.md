...
| Date | Version | Feature | Description | Commit | Notes |
|---|---|---|---|---|---|
| 2026-08-04 | v0.4.1 | `dashboard-filters-row` | Ajuste visual: filtros del dashboard (selector URL/Query + 3 botones Importar / Generar predicción / Patrones) en una sola fila flex row, eliminando el cell vacío del grid anterior (`grid-cols-3`). En desktop, selector y grupo de botones comparten fila horizontal sin hueco intermedio. En mobile, se apilan vertical. Sin cambios funcionales (lógica de `x-show`/`x-model`/`@click` intacta). Docs: nueva sub-sección §4.7 "Filas de acciones" en design-system con el patrón, las clases Tailwind y los anti-patrones a evitar. | `666bd27`+`8f6c6bc` | 2 commits en development (frontend + docs). Refs: [`docs/design-system.md §4.7`](design-system.md#47-filas-de-acciones-filtros-contextuales--grupo-de-botones), [`frontend/index.html`](../frontend/index.html). |

### v0.4.0 — Chart enhancements (annotations + donut + sparklines)

| Date | Version | Feature | Description | Commit | Notes |
|---|---|---|---|---|---|
| 2026-08-04 | v0.4.0 | `chart-enhancements` | Tres enhancements visuales sobre la gráfica y stat cards del dashboard, sin tocar la lógica de predicción ni de alertas: (1) Annotations de `external_events` en la gráfica de tráfico (líneas verticales por fecha, color rojo para `update_google` y amber para `mercado`), filtradas al rango visible; (2) Donut de alertas por severidad (alta/media/baja) en la sección "Alertas activas", con leyenda abajo y colores alineados a los badges existentes; (3) Sparklines en 3 stat cards (alertas, eventos, acciones registradas) usando ApexCharts `sparkline.enabled: true`, alimentadas por el nuevo endpoint `GET /api/dashboard/sparklines?accountId=N&days=14` que devuelve series temporales de 14 días (cuenta por día para alerts/events/strategy). `updateSeries`/`updateOptions` en lugar de destroy+render cuando ya existe instancia. | `eed4a4d`+`eec306e`+`edb59aa` | 3 commits en development (backend + frontend + docs). Refs: [`docs/design-system.md §6`](design-system.md#6-gráfica-apexcharts), [`backend/routes/dashboard.js`](../backend/routes/dashboard.js), [`frontend/js/dashboard.js`](../frontend/js/dashboard.js). |

### v0.3.0 — Visual refresh estilo Flowbite Admin Dashboard

| Date | Version | Feature | Description | Commit | Notes |
|---|---|---|---|---|---|
| 2026-08-04 | v0.3.0 | `ui-flowbite-redesign` | Rediseño visual completo siguiendo el lenguaje de Flowbite Admin Dashboard: shell con sidebar w-64 + iconos Lucide + topbar sticky con breadcrumb/search/user dropdown; dashboard con stat cards row; connect con step indicator; accounts con tabla row hover + iconos en acciones + modales con icon header; login con brand header + iconos en inputs. Layout sticky: sidebar + topbar fijos, solo el content slot scrollea. Sin agregar Flowbite como dependencia — replicado con utility classes de Tailwind + Lucide via CDN. Documentación: `docs/design-system.md` actualizado con §0 (inspiración), §8 (iconografía), §9 (referencia al page-template); nuevo `docs/page-template.md` con el template canónico para páginas nuevas. | `d5d946b`+`61036d7`+`dd47a80`+`7a67fa6`+`c286930` | 5 commits en development. Refs: [`docs/page-template.md`](page-template.md), [`docs/design-system.md`](design-system.md#8-iconografía-con-lucide). |
